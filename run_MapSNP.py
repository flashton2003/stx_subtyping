	...
	for each in mapped_bases:
		...
		if each in uniq_pos:
			...
			if depth > 5:
				...
				if uniq_mapping / depth >= 0.2:
					i += 1
					#print each, mapped_bases[each]

	if i == 3:
		return '%s is present in sample' % reference

			#print each, mapped_bases[each]


def run_pileup_analysis(bwa_output_dir, s_name):
	...
	for reference in references:
		...
		parse_pileup(mapped_bases, reference)




'''
Next step; need to add in the make_pile_up and come up with a better way to parse this to get the subtype specific snps
also, should find snps that are specific for all subtypes - will need to map (possibly pseudo fastqs), to the reference set and see which regions map unambigously
'''
#################################### main ####################################
...
run_pileup_analysis(bwa_output_dir, s_name)
